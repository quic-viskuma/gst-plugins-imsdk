...
struct VLMAppContext {
  ...
  // A vector of appsrc_main elements for all pipelines for easier lookup
...
vlm_init (VLMAppContext * appctx)
{
  ...
  appctx->vlm = new GenieVLM (cfg, [] (uint64_t id, bool ok,
      const VlmResult & r,
...
create_pipeline_rz (VLMAppContext * appctx, guint idx)
{
  ...
  /* -------- VLM tensor branch to appsink_vlm --------
     ...
     In parse_launch we can set only format/type easily; dimensions are tricky as string.
     Keep minimal caps same as you used (format/type). */
  appendf (out, OUT_SZ,
    "tee1-%u. ! "
