...
#define GST_BUFFER_ITERATE_ROI_METAS(buffer, state)                   \
  (GstVideoRegionOfInterestMeta *) gst_buffer_iterate_meta_filtered ( \
...
#define DEFAULT_THRESHOLD_VALUE 60.0

#define RZ_DETECTION_MODEL "/etc/models/foot_track_net_quantized.tflite"
#define RZ_DETECTION_LABELS "/etc/models/foot_track_net.json"
#define RZ_DETECTION_SETTINGS "/etc/models/foot_track_net_settings.json"
...
#define REINIT_VLM_ON_FAILURE_CNT 5
...
typedef std::pair<guint, guint> Coordinates;
typedef std::vector<Coordinates> ZonePolygon;
...
struct VLMAppContext {
  ...
  // Map to store restricted areas on screen
  std::map<std::string, ZonePolygon> restricted_areas; ////
  ...
  std::vector<GstElement *> appsrces_main;
  // A vector of appsrc_vlm elements for all pipelines for easier lookup
  std::vector<GstElement *> appsrces_vlm;
  ...
  gboolean attach_jpeg = FALSE;
  guint process_interval_ms = 3000;
  guint bbox_threshold = 8;
  gdouble bbox_margin = 1.0;
  gboolean zone_detection;
  GstClockTime lasttime;
  gboolean first_res_sent;
  //          **** IPC related members ****
  IpcStream stream;
  GThread * vlm_poll_thread;
  std::atomic<guint> reinit_VLM_cnt{0};
  std::atomic<guint64> next_id{1};
  std::atomic<guint64> last_rx_ms{0};
  std::atomic<bool> poll_thread_exit{false};
  std::unordered_map<guint64, VLMBufferBundle *> vlm_requests;
  std::unordered_map<guint64, guint64> pending_ms; // client_id -> send_time_ms
  std::mutex io_mtx;                               // protects stream.in/out
  std::mutex pending_mtx;                          // protects pending_ms
  pid_t worker_pid;
  int ipc_fd;
};
...
auto now_ms = [] () -> guint64 {
  return (guint64) std::chrono::duration_cast<std::chrono::milliseconds> (
...
/**
 * Build VLM engine configuration and apply options based on app context
 *
 * @param appctx Pointer to App context object
 * @param logger_choice Requested log level
 * @param profile_enabled Flag indicating eprformance profiling on/off
 *
 */
static GenieVLMConfig
make_default_pool_config (VLMAppContext * appctx,
  const std::string & logger_choice,
...
/**
 * Runs worker-side VLM processing loop, receiving and submitting requests over
 * IPC
 *
 * @param appctx Pointer to App context object
 * @param ipc_fd File descriptor used for IPC between worker and main process
 * @param logger_choice Requested log level
 * @param profile_enabled Flag indicating eprformance profiling on/off
 *
 */
static int
run_worker (VLMAppContext * appctx, int ipc_fd,
  const std::string & logger_choice, bool profile_enabled)
{
  std::mutex map_mtx;
  std::unordered_map<guint64, guint64> id_map; // internal_id -> client_id

  GenieVLMConfig cfg =
      make_default_pool_config (appctx, logger_choice, profile_enabled);

  GenieVLM pool (cfg, [&] (guint64 internal_id, bool ok, const VlmResult & r,
      const std::string & err) {
    guint64 client_id = 0;
    {
      std::lock_guard<std::mutex> lk (map_mtx);
      auto it = id_map.find (internal_id);
      if (it != id_map.end ()) {
        client_id = it->second;
        id_map.erase (it);
      }
    ...
    ipc::RespHeader hdr{};
    hdr.type = ipc::MSG_RESPONSE;
    hdr.ok = ok ? 1u : 0u;
    hdr.client_id = client_id;
    hdr.text_len = static_cast<guint> (r.text.size ());
    hdr.err_len = static_cast<guint> (err.size ());
    hdr.userdata = r.user_data;

    if (!ipc::write_all (ipc_fd, &hdr, sizeof (hdr)))
      return;

    if (!r.text.empty ())
      (void) ipc::write_all (ipc_fd, r.text.data (), r.text.size ());

    if (!err.empty ())
      (void) ipc::write_all (ipc_fd, err.data (), err.size ());
  });

  while (true) {
    ipc::ReqHeader hdr{};
    if (!ipc::read_all (ipc_fd, &hdr, sizeof (hdr))) {
      return 0; // client closed
    }

    if (hdr.type != ipc::MSG_REQUEST) {
      return 2;
    }
    ...
    assistant.resize (hdr.assistant_len);
    question.resize (hdr.question_len);
    buf.resize (hdr.buf_len);

    if (hdr.assistant_len &&
        !ipc::read_all (ipc_fd, assistant.data (), hdr.assistant_len))
      return 3;

    if (hdr.question_len &&
        !ipc::read_all (ipc_fd, question.data (), hdr.question_len))
      return 3;

    if (hdr.buf_len && !ipc::read_all (ipc_fd, buf.data (), hdr.buf_len))
      return 3;

    guint64 internal_id =
      pool.submit (buf.data (), std::move (assistant), std::move (question),
        hdr.temperature, hdr.top_p, hdr.seed, hdr.presence_penalty,
        hdr.frequency_penalty, hdr.userdata);

    if (internal_id == 0) {
      const std::string e = "submit rejected (backpressure)";
      ipc::RespHeader rh{};
      rh.type = ipc::MSG_RESPONSE;
      rh.ok = 0;
      rh.client_id = hdr.client_id;
      rh.text_len = 0;
      rh.err_len = static_cast<guint> (e.size ());
      rh.userdata = hdr.userdata;
      if (!ipc::write_all (ipc_fd, &rh, sizeof (rh)))
        return 0;
      (void) ipc::write_all (ipc_fd, e.data (), e.size ());
      continue;
    }

    {
      std::lock_guard<std::mutex> lk (map_mtx);
      id_map.emplace (internal_id, hdr.client_id);
    }
  }
}

/**
 * Creates a worker process and starts it with the proper arguments and IPC fd
 *
 * @param self_path Path to current executable, used to re-exec the worker
 * @param config_path Path to the application's config file
 * @param out_ipc_fd Output pointer that receives the IPC fd in the parent
 * process
 * @param logger_choice Requested log level
 * @param profile_enabled Flag indicating eprformance profiling on/off
 *
 */
static pid_t
spawn_worker_process (const char * self_path, const char * config_path,
  int * out_ipc_fd, const std::string & logger_choice,
  bool profile_enabled)
{
  int sv[2];
  ...
  if (pid == 0) {
    close (sv[0]);
    const int target_fd = 3;
    if (sv[1] != target_fd) {
      dup2 (sv[1], target_fd);
      close (sv[1]);
    }
    ipc::set_fd_cloexec (target_fd, false);

    std::vector<std::string> args;
    args.emplace_back (self_path);
    args.emplace_back ("--worker-fd");
    args.emplace_back ("3");
    args.emplace_back ("--config");
    args.emplace_back (config_path);
    args.emplace_back ("--logger");
    args.emplace_back (logger_choice);
    args.emplace_back (profile_enabled ? "--profile" : "--no-profile");
...
/**
 * Attempts to gracefully terminate a worker process and reap its exit status
 *
 * @param pid PID of the process to terminate and reap
 *
 */
static void
...
/**
 * Checks whether a worker process already exited without blocking
 *
 * @param pid PID of the process to test for liveness
 *
 */
static bool
worker_is_dead (pid_t pid)
{
  if (pid <= 0)
    return true;
  int st = 0;
...
/**
 * Closes the IPC file descriptor and terminates the worker process if it runs
 *
 * @param io_fd Pointer to the worker's IPC fd
 * @param io_pid Pointer to the worker's PID
 *
 */
static bool
stop_worker (int * io_fd, pid_t * io_pid)
{
...
/**
 * Stops the current worker process and starts a new one, replacing its IPC fd
 * and PID
 *
 * @param self_path Path to the executable used to launch the worker
 * @param config_path Config file path passed to the new worker
 * @param io_fd Pointer to store the new IPC fd
 * @param io_pid Pointer to store the new worker's PID
 * @param logger_choice Requested log level
 * @param profile_enabled Flag indicating eprformance profiling on/off
 *
 */
static bool
restart_worker (const char * self_path, const char * config_path, int * io_fd,
  pid_t * io_pid, const std::string & logger_choice,
  bool profile_enabled)
{
  if (io_fd && *io_fd >= 0) {
    close (*io_fd);
  ...
  int new_fd = -1;
  pid_t new_pid = spawn_worker_process (self_path, config_path, &new_fd,
                                        logger_choice, profile_enabled);

  if (new_pid <= 0 || new_fd < 0) {
    if (new_fd >= 0)
      close (new_fd);
    return false;
  }
...
/**
 * Fully resets the worker environment, flushing IO queues, restarting the
 * worker process and reinitializing IPC state
 *
 * @param appctx Pointer to the App context object
 *
 */
void
hard_restart (VLMAppContext * appctx)
{
  g_printerr ("Restarting worker...\n");

  // 1) Drop all pending requests and free their buffers/bundles
  std::vector<VLMBufferBundle *> to_free;
  {
    std::lock_guard<std::mutex> lk (appctx->pending_mtx);
    appctx->pending_ms.clear ();

    g_printerr ("Remove VLM requests - %ld\n", appctx->vlm_requests.size ());
    to_free.reserve (appctx->vlm_requests.size ());
    for (const auto & kv : appctx->vlm_requests) {
      to_free.push_back ((VLMBufferBundle *) kv.second);
    }
    appctx->vlm_requests.clear ();
  }

  for (VLMBufferBundle * b : to_free) {
    if (!b)
      continue;
    if (b->jpeg_buffer)
      gst_buffer_unref (b->jpeg_buffer);
    if (b->vlm_buffer)
      gst_buffer_unref (b->vlm_buffer);
    g_slice_free (VLMBufferBundle, b);
  }

  // 2) Clear IO queues
  {
    std::lock_guard<std::mutex> lk (appctx->io_mtx);
    appctx->stream.in.clear ();
    appctx->stream.out.clear ();
  }

  // 3) Restart the worker process
  if (!restart_worker ("/usr/bin/gst-vlm-rz-usecase-ipc", appctx->config_file,
      &appctx->ipc_fd, &appctx->worker_pid,
      appctx->logger_level, appctx->profile_enabled)) {
    g_printerr ("Worker restart failed\n");
    exit (2);
  }

  ipc::set_fd_nonblock (appctx->ipc_fd, true);
  appctx->stream.fd = appctx->ipc_fd;
  appctx->last_rx_ms.store (now_ms ());
}

/**
 * Submit an inference request to the worker process via IPC
 * @param appctx Pointer to the App context object
 * @param buffer Pointer to the raw buffer data
 * @param asistant_description Assistant prompt for model
 * @param temperature Temperature for text generation
 * @param top_p Nucleus sampling parameter controlling diversity
 * @param seed Random seed for deterministic or varied input
 * @param presence_penalty Penalty to discourage repeating concepts
 * @param frequency_penalty Penalty to reduce repeated tokens
 * @param user_data Pointer forwarded through the IPC
 *
 */
static guint64
ipc_submit (VLMAppContext * appctx, guint8 * buffer, guint64 size,
  std::string asistant_description, std::string question,
  float temperature, float top_p, float seed, float presence_penalty,
  float frequency_penalty, void * user_data)
{
  // allow sending new requests without waiting
  const size_t max_inflight = appctx->vlm_engines.size ();

  if (appctx->poll_thread_exit.load ()) {
    g_printerr ("Poll thread not running\n");
    return 0;
  }

  // 1) Detect dead worker
  if (worker_is_dead (appctx->worker_pid)) {
    g_printerr ("Worker exited\n");
    hard_restart (appctx);
  }

  // 2) Respect inflight limit (non-blocking)
  size_t inflight = 0;
  {
    std::lock_guard<std::mutex> lk (appctx->pending_mtx);
    inflight = appctx->pending_ms.size ();
  }
  if (inflight >= max_inflight)
    return 0;

  // NOTE: this still copies. If you want zero-copy later, we can pass a SHM
  // handle or memfd.
  std::vector<guint8> buf (buffer, buffer + size);

  const guint64 cid = appctx->next_id.fetch_add (1);

  // enqueue_request touches stream.out -> must be serialized with IO thread
  {
    std::lock_guard<std::mutex> lk (appctx->io_mtx);
    appctx->stream.enqueue_request (cid, buf, asistant_description, question,
      temperature, top_p, seed, presence_penalty,
      frequency_penalty, user_data);
  }

  {
    std::lock_guard<std::mutex> lk (appctx->pending_mtx);
    appctx->pending_ms.emplace (cid, now_ms ());
    appctx->vlm_requests.emplace (cid, (VLMBufferBundle *) user_data);
  }
  return cid;
}

/**
 * Runs the IPC I-O loop that polls the worker process, sends pending requests
 * and triggers automatic worker restarts on errors/timeouts
 *
 * @param appctx Pointer to the App context object
 *
 */
void
ipc_pool_thread_fn (VLMAppContext * appctx)
{
  while (1) {
    if (appctx->poll_thread_exit.load ()) {
      g_print ("Poll thread exit\n");
      break;
    }

    // Re-init counter is decremented on repeated VLM failures (see response
    // handler).
    if (appctx->reinit_VLM_cnt.load () == 0) {
      appctx->reinit_VLM_cnt.store (REINIT_VLM_ON_FAILURE_CNT);
      g_printerr ("Reinit VLM\n");
      hard_restart (appctx);
      continue;
    }

    // 3) Poll for IO readiness
    struct pollfd pfd;
    pfd.fd = appctx->stream.fd;
    pfd.events = POLLIN;
    {
      std::lock_guard<std::mutex> lk (appctx->io_mtx);
      if (!appctx->stream.out.empty ())
        pfd.events |= POLLOUT;
    }
    pfd.revents = 0;
    ...
    if (pr < 0) {
      if (errno == EINTR)
        continue;
      g_printerr ("poll () failed - restarting\n");
      hard_restart (appctx);
      continue;
    }

    if (pfd.revents & (POLLERR | POLLHUP | POLLNVAL)) {
      g_printerr ("IPC fd error/hup - restarting\n");
      hard_restart (appctx);
      continue;
    }
    ...
    if (pfd.revents & POLLOUT) {
      std::lock_guard<std::mutex> lk (appctx->io_mtx);
      if (!appctx->stream.out.empty () && !appctx->stream.flush_out ()) {
        g_printerr ("write to worker failed - restarting\n");
        hard_restart (appctx);
        continue;
      }
    }

    // 5) Read and parse responses
    if (pfd.revents & POLLIN) {
      {
        std::lock_guard<std::mutex> lk (appctx->io_mtx);
        if (!appctx->stream.pump_in ()) {
          g_printerr ("read from worker failed - restarting\n");
          hard_restart (appctx);
          continue;
        }
      }
      while (true) {
        ...
        {
          std::lock_guard<std::mutex> lk (appctx->io_mtx);
          got = appctx->stream.try_pop_response (rid, ok, text, err, &userdata);
        ...
        if (!got) {
          if (errno == EPROTO) {
            g_printerr ("Protocol error - restarting\n");
            hard_restart (appctx);
          }
          break;
        }
        ...
        {
          std::lock_guard<std::mutex> lk (appctx->pending_mtx);
          if (appctx->vlm_requests.count (rid) == 0) {
            g_printerr ("Missing request - skip\n");
            continue;
          }

          appctx->pending_ms.erase (rid);
          appctx->vlm_requests.erase (rid);
        }

        if (ok) {
          std::string result;
          VLMBufferBundle * vlm_buf_bundle = (VLMBufferBundle *) userdata;
          ...
          if (result.empty () || result.size () < 5) {
            ...
            {
              guint cur = vlm_buf_bundle->appctx->reinit_VLM_cnt.load ();
              if (cur > 0)
                vlm_buf_bundle->appctx->reinit_VLM_cnt.store (cur - 1);
            }
            goto exit;
          }
          ...
          appsink_ml_meta_buf =
              gst_buffer_new_allocate (NULL, result.size (), NULL);
          ...
          gst_buffer_fill (appsink_ml_meta_buf, 0, result.c_str (),
            result.size ());

          GST_BUFFER_PTS (appsink_ml_meta_buf) = gst_util_get_timestamp ();
          GST_BUFFER_DTS (appsink_ml_meta_buf) =
              GST_BUFFER_PTS (appsink_ml_meta_buf);

          g_mutex_lock (&vlm_buf_bundle->appctx->mutex);

          // Push JPEG buffer
          if (vlm_buf_bundle->appsrc_jpeg && vlm_buf_bundle->jpeg_buffer)
          ...
          // Push Meta buffer
          if (vlm_buf_bundle->appsrc_vlm)
            push_buffer_to_appsrc (vlm_buf_bundle->appsrc_vlm,
              appsink_ml_meta_buf);
          else
            g_printerr ("appsrc_vlm is NULL - drop meta\n");
    ...
    // 6) Stuck detection: any pending older than timeout OR no RX while pending
    {
      bool do_restart = FALSE;
      {
        std::lock_guard<std::mutex> lk (appctx->pending_mtx);
        if (!appctx->pending_ms.empty ()) {
          const guint64 tnow = now_ms ();
          bool timeout = false;
          for (const auto & kv : appctx->pending_ms) {
            if (tnow - kv.second > RESPONSE_TIMEOUT_MS) {
              timeout = true;
              break;
            }
          }
          if (timeout ||
              (tnow - appctx->last_rx_ms.load () > RESPONSE_TIMEOUT_MS)) {
            do_restart = TRUE;
          }
        }
      }
      ...
      if (do_restart) {
        g_printerr ("Worker appears stuck (pending timeout)\n");
        hard_restart (appctx);
      }
    }

    // 7) Pace submissions a bit
    usleep (50000);
  }
}
...
parse_json (const gchar * config_file, VLMAppContext * appctx)
{
  ...
  GError * error = NULL;
  JsonArray *input_file_path_array = NULL, *zones_array = NULL,
            *zone_array = NULL;
  guint num_streams, num_zones;
  ...
  if (json_object_has_member (root_obj, "sources")) {
    ...
    for (guint i = 0; i < num_streams; i++) {
      ...
      // Guard against missing/NULL members (json-glib returns NULL
      // and std::string ctor would crash)
  ...
  if (json_object_has_member (root_obj, "zones")) {
    zones_array = json_object_get_array_member (root_obj, "zones");
    num_zones = json_array_get_length (zones_array);

    for (guint i = 0; i < num_zones; i++) {
      std::string zone_name;
      guint zone_array_len;
      ZonePolygon polygon;
      JsonObject * zone = json_array_get_object_element (zones_array, i);

      zone_name = json_object_get_string_member (zone, "name");
      zone_array = json_object_get_array_member (zone, "coords");
      zone_array_len = json_array_get_length (zone_array);

      for (guint j = 0; j < zone_array_len; j++) {
        Coordinates coordinates;

        JsonObject * coordinates_obj =
          json_array_get_object_element (zone_array, j);

        coordinates.first = json_object_get_int_member (coordinates_obj, "x");
        coordinates.second = json_object_get_int_member (coordinates_obj, "y");

        polygon.push_back (coordinates);
      }
      appctx->restricted_areas[zone_name] = polygon;
    }
  }
  ...
  if (json_object_has_member (root_obj, "zone_detection"))
    appctx->zone_detection =
      json_object_get_boolean_member (root_obj, "zone_detection");
...
on_meta_new_sample (GstElement * appsink, gpointer user_data)
{
  ...
  if (idx >= ud->appctx->data_channels.size () ||
    ...
    g_print ("on_meta_new_sample no data channel\n");
...
gst_app_context_free (VLMAppContext * appctx)
{
  ...
  for (guint i = 0; i < appctx->num_streams; i++) {
    ...
    if (i < appctx->appsrces_main.size () && appctx->appsrces_main[i]) {
      gst_object_unref (appctx->appsrces_main[i]);
      appctx->appsrces_main[i] = NULL;
    }
    if (i < appctx->appsrces_vlm.size () && appctx->appsrces_vlm[i]) {
      gst_object_unref (appctx->appsrces_vlm[i]);
      appctx->appsrces_vlm[i] = NULL;
    }
...
{
  g_print ("Client mode: starting VLM worker process...\n");
  appctx->worker_pid = spawn_worker_process (
      "/usr/bin/gst-vlm-rz-usecase-ipc", appctx->config_file, &appctx->ipc_fd,
      appctx->logger_level, appctx->profile_enabled);

  if (appctx->worker_pid <= 0 || appctx->ipc_fd < 0) {
    g_printerr ("Failed to spawn worker process\n");
    return FALSE;
  }
...
{
  g_print ("Stopping worker\n");
  stop_worker (&appctx->ipc_fd, &appctx->worker_pid);
}
...
process_vlm (VLMAppContext * appctx, guint pipeline_idx)
{
  ...
  if (appctx->buffer_bundles[pipeline_idx].vlm_buffer == NULL ||
    ...
    g_printerr ("ERROR: process_vlm missing buffers.\n");
    return;
  }
  ...
  req_id = ipc_submit (appctx, memmap.data, memmap.size,
                       appctx->assistant_query, question,
                       1.5, // temperature
                       0.5, // top_p
                       0.0, // seed
...
/**
 * Expands a bounding box by a margin factor, also ensuring it remains within
 * boundaries.
 *
 * @param roimeta ROI metadata whose bounding box should be expanded.
 * @param frame_w  Frame width
 * @param frame_h  Frame height
 * @param margin  Expansion ratio applied to the bounding box size.
 *
 */
static void
apply_bbox_margin (GstVideoRegionOfInterestMeta * roimeta, gint frame_w,
  gint frame_h, gdouble margin)
{
  if (!roimeta)
    return;
  if (margin <= 0.0)
  ...
  // Expand bbox by a factor around its center.
  // margin = 1.0 => +100% (i.e. 2x size). margin = 0.2 => +20%
  gint x = roimeta->x;
  gint y = roimeta->y;
  gint w = roimeta->w;
  gint h = roimeta->h;
  ...
  gdouble new_w = w * (1.0 + margin);
  gdouble new_h = h * (1.0 + margin);

  gint nx = (gint)(cx - new_w * 0.5);
  gint ny = (gint)(cy - new_h * 0.5);
  gint nw = (gint)(new_w);
  gint nh = (gint)(new_h);
...
/**
 * Filters, adjusts, and removes ROI metadata from a video buffer
 *
 * @param appsink Pointer to the App context object
 * @param user_data A pointer to buffer containing the ROI meta
 *
 */
gboolean
process_rois_from_buffer (VLMAppContext * appctx, GstBuffer * buffer)
{
  ...
  const guint coeff =
      (appctx->bbox_threshold > 0) ? appctx->bbox_threshold : 10;
  const double min_w = (double) vmeta->width / (double) coeff;
  const double min_h = (double) vmeta->height / (double) coeff;

  const GstClockTime interval_ns =
    (appctx->process_interval_ms > 0)
    ? ((GstClockTime) appctx->process_interval_ms * GST_MSECOND)
    : (4 * GST_SECOND);
  ...
  while ((roimeta = GST_BUFFER_ITERATE_ROI_METAS (buffer, state)) != NULL) {
    ...
    // margin
    apply_bbox_margin (roimeta, vmeta->width, vmeta->height,
      appctx->bbox_margin);

    // Drop small ROIs
    if ((double) roimeta->w < min_w || (double) roimeta->h < min_h) {
      g_ptr_array_add (to_remove, roimeta);
      continue;
    }

    // Parse ObjectDetection
    ...
    // zone detection
    if (appctx->zone_detection) {
      if (gst_structure_has_field (objparam, "zones")) {
        const GValue * zones = gst_structure_get_value (objparam, "zones");
        if (gst_value_array_get_size (zones) == 0) {
          g_ptr_array_add (to_remove, roimeta);
          continue;
        }
      } else {
    ...
    // interval gate
    if (appctx->lasttime != GST_CLOCK_TIME_NONE &&
        GST_CLOCK_DIFF (appctx->lasttime, now) <= (gint64) interval_ns &&
        appctx->first_res_sent) {
      g_ptr_array_add (to_remove, roimeta);
      continue;
    }

    appctx->lasttime = now;
    appctx->first_res_sent = TRUE;
  }

  // Remove unwanted ROIs
  for (guint i = 0; i < to_remove->len; i++)
    gst_buffer_remove_meta (buffer,
      (GstMeta *) g_ptr_array_index (to_remove, i));

  if (to_remove->len == meta_cnt) {
    // All metas are removed, no metas to process further
    g_ptr_array_free (to_remove, TRUE);
    return FALSE;
  }
...
GstFlowReturn
appsink_main_callback (GstElement * appsink, gpointer user_data)
{
  GstSample * sample = NULL;
  ...
  if (sample) {
    ...
    if (process_rois_from_buffer (appctx, buffer)) {
      push_buffer_to_appsrc (appctx->appsrces_main[pipeline_idx], buffer);
    }
  }
  return GST_FLOW_OK;
}
  ...
  if (sample) {
    ...
    if (appctx->buffer_bundles[pipeline_idx].vlm_buffer == NULL) {
      ...
      appctx->buffer_bundles[pipeline_idx].appsrc_vlm =
          appctx->appsrces_vlm[pipeline_idx];
...
/**
 * Updates the provided capsfilter with newly negotiated caps reported by the
 * appsink pad.
 *
 * @param pad The appsink pad whose caps have changed
 * @param pspec Parameter specification for the changed property
 * @param user_data Pointer to the capsfilter element to receive the updated
 * caps.
 *
 */
void
on_appsink_caps_notify (GObject * pad, GParamSpec * pspec, gpointer user_data)
{
  (void) pspec;
...
/**
 * Serialize the data regarding detection zone coordinates
 *
 * @param appctx Application Context object
 *
 */
std::string
serialize_zones_data (VLMAppContext * appctx)
{
  std::string restricted_zone_string;
  restricted_zone_string += "Zones";

  for (auto it = appctx->restricted_areas.begin ();
      it != appctx->restricted_areas.end (); it++) {
    gchar restricted_ch_buf[16];

    snprintf (restricted_ch_buf, 16, ",%s=<", it->first.c_str ());
    restricted_zone_string += restricted_ch_buf;

    ZonePolygon * polygon = &it->second;
    for (guint i = 0; i < polygon->size (); i++) {
      Coordinates * c = &polygon->at (i);
      snprintf (restricted_ch_buf, 16, "<%u,%u>", c->first, c->second);

      restricted_zone_string += restricted_ch_buf;
      if (i + 1 < polygon->size ())
        restricted_zone_string += ",";
    }
    restricted_zone_string += ">";
  }
  restricted_zone_string += ";";

  return restricted_zone_string;
}

/**
 * Serialize the data regarding overlay zone coordinates
 *
 * @param appctx Application Context object
 *
 */
std::string
serialize_zones_data_overlay (VLMAppContext * appctx)
{
  std::string out;
  out += "{";
  ...
  for (auto it = appctx->restricted_areas.begin ();
    ...
    // (structure)"Zone1,polygon=<<...>>,color=0x....;"
    out += "(structure)\"";
    ...
    for (guint i = 0; i < polygon->size (); ++i) {
      ...
      // Enough for "<4294967295,4294967295>" + '\0' => 23 chars.
      // Use bigger buffer to be safe.
      char buf[64];
      snprintf (buf, sizeof (buf), "<%u,%u>", c->first, c->second);
      out += buf;
...
/**
 * A function to create a RZ pipeline:
 *
 ...
 */
static GstElement *
create_pipeline_rz (VLMAppContext * appctx, guint idx)
{
  ...
  std::string restricted_zone_string;
  std::string restricted_zone_string_overlay;

  if (!appctx->restricted_areas.empty ()) {
    restricted_zone_string = serialize_zones_data (appctx);
    g_print ("(Stream %u) RZ zones - %s\n", idx,
      restricted_zone_string.c_str ());

    restricted_zone_string_overlay = serialize_zones_data_overlay (appctx);
    g_print ("(Stream %u) RZ overlay zones - %s\n", idx,
      restricted_zone_string_overlay.c_str ());
  } else
    g_print ("(Stream %u) No restricted areas information was provided, or "
      "it was not properly set in config JSON\n",
      idx);
  ...
  /* -------- DET chain -------- */
  appendf (out, OUT_SZ,
    "tee0-%u. ! queue ! qtimetamux name=qtimetamux_det-%u "
    "tee0-%u. ! "
    "qtimlvconverter ! "
    "queue ! "
    "qtimltflite "
    ...
    "queue ! "
    "qtimlpostprocess "
      "bbox-stabilization=true results=5 module=qpd labels=\"%s\" "
      "settings=\"%s\" ! "
    "text/x-raw ! "
    "queue ! "
    "qtimetamux_det-%u. "
    "qtimetamux_det-%u. ! queue ! ",
    idx,
    idx,
    idx,
    RZ_DETECTION_MODEL, 1,
    RZ_DETECTION_LABELS, RZ_DETECTION_SETTINGS,
    idx,
    idx
  );
  ...
  /* -------- qtivsplit_main -------- */
  appendf (out, OUT_SZ,
    "qtivsplit name=qtivsplit_main-%u "
    "src_0::mode=1 "
    "src_1::mode=1 ",
    idx
  );

  /* -------- tracker + restricted zone + qtivsplit_roi + tee1 -------- */
  appendf (out, OUT_SZ,
    "qtivsplit_main-%u. ! queue ! "
    "qtirestrictedzonedbg zone-config=\"%s\" ! "
    "appsink name=appsink_main-%u sync=false emit-signals=true async=false enable-last-sample=false ",
    idx, restricted_zone_string.c_str(), idx
  );

  /* -------- appsrc_main + caps + qtivsplit_roi + tee1 -------- */
  appendf (out, OUT_SZ,
    "appsrc name=appsrc_main-%u ! "
    "capsfilter name=appsrc_main_caps-%u caps=video/x-raw,format=NV12 ! "
    "qtivsplit name=qtivsplit_roi-%u "
    "src_0::mode=3 src_1::mode=3 ! "
    "queue ! "
    "tee name=tee1-%u ",
    idx,
    idx,
  ...
  /* -------- restricted zone overlay + webrtcbin -------- */
  appendf (out, OUT_SZ,
    "qtivsplit_main-%u. ! queue ! "
    "qtirestrictedzonedbg zone-config=\"%s\" ! "
    "qtivoverlay masks=\"%s\" ! "
    ...
    "webrtcbin latency=100 name=webrtcbin-%u "
      ...
      "bundle-policy=3 ",
    idx, restricted_zone_string.c_str (), restricted_zone_string_overlay.c_str (),
        enc_str.c_str (), idx
  );
  ...
  /* -------- qtimetamux_vlm -> metaparser -> meta_sink -------- */
  appendf (out, OUT_SZ,
    "qtimetamux_vlm-%u. ! "
...
create_pipeline_det (VLMAppContext * appctx, guint idx)
{
  ...
  appendf (out, OUT_SZ,
    ...
    "tee0-%u. ! "
    "videorate drop-only=true ! video/x-raw,framerate=10/1 ! queue ! "
    "qtimlvconverter ! "
    "queue ! "
    "qtimltflite "
      "model=\"%s\" delegate=external "
...
main (gint argc, gchar * argv[])
{
  ...
  appctx.zone_detection = FALSE;
  appctx.lasttime = GST_CLOCK_TIME_NONE;
  appctx.first_res_sent = FALSE;
  ...
  appctx.stream.in.clear ();
  appctx.stream.out.clear ();
  appctx.vlm_poll_thread = NULL;
  appctx.poll_thread_exit.store (false);
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (!g_strcmp0 (argv[i], "--worker-fd")) {
      if (i + 1 < argc) {
        worker_fd = std::atoi (argv[++i]);
      } else {
        g_printerr ("Missing argument after %s\n", argv[i]);
        return -1;
      }
  ...
  if (worker_fd >= 0) {
    return run_worker (&appctx, worker_fd, appctx.logger_level,
      appctx.profile_enabled);
  }
  ...
  appctx.appsrces_main.resize (appctx.num_streams);
  appctx.appsrces_vlm.resize (appctx.num_streams);
  ...
  while (TRUE) {
    ...
    // Figure out what needs to be in the loop
    for (guint i = 0; i < appctx.num_streams; i++) {
      ...
      if (appctx.stream_configs[i].pipeline_type == "rz") {
        pipeline = create_pipeline_rz (&appctx, i);
      ...
      if (appctx.stream_configs[i].pipeline_type == "rz") {
        ...
        // during stream
        snprintf (name, 127, "appsrc_main-%u", i);
        appctx.appsrces_main[i] =
            gst_bin_get_by_name (GST_BIN (pipeline), name);
        snprintf (name, 127, "appsrc_vlm-%u", i);
        appctx.appsrces_vlm[i] = gst_bin_get_by_name (GST_BIN (pipeline), name);
        ...
        snprintf (name, 127, "appsink_main-%u", i);
        elem = gst_bin_get_by_name (GST_BIN (pipeline), name);
        g_signal_connect (G_OBJECT (elem), "new-sample",
          G_CALLBACK (appsink_main_callback), &appctx);

        snprintf (name, 127, "appsrc_main_caps-%u", i);
        ...
        g_signal_connect (sinkpad, "notify::caps",
          G_CALLBACK (on_appsink_caps_notify), elem2);
    ...
    std::thread io_thread (ipc_pool_thread_fn, &appctx);
    g_print ("Start pooling thread ...\n");
  ...
  {
    std::lock_guard<std::mutex> lk (appctx.io_mtx);
    appctx.poll_thread_exit.store (true);
    appctx.stream.in.clear ();
    appctx.stream.out.clear ();
  }
    // Stop IO thread before touching pending maps/buffers
    ...
    // Stop worker (closes fd and reaps child)
    vlm_deinit (&appctx);
    ...
    g_mutex_lock (&appctx.mutex);
    const gboolean should_exit = appctx.exit;
    g_mutex_unlock (&appctx.mutex);
