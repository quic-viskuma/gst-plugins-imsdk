 ...
 * @param logger_choice Requested log level
 * @param profile_enabled Flag indicating performance profiling on/off
 *
 */
static bool
...
hard_restart (VLMAppContext * appctx)
{
  ...
  if (!restart_worker ("/usr/bin/gst-vlm-multistream-usecase-ipc",
      appctx->config_file, &appctx->ipc_fd,
      &appctx->worker_pid, appctx->logger_level,
  ...
  float frequency_penalty, void * user_data)
{
  const size_t max_inflight =
      appctx->vlm_engines.size (); // allow sending new requests without waiting
  ...
  g_print ("Enqueue request: size = %ld\n", size);
  return cid;
}
...
scene_det_process_gray8 (SceneDet * sd, const guint8 * data, gint stride,
  gint w, gint h)
{
  gfloat cur[64];
...
vlm_init (VLMAppContext * appctx)
{
  ...
  appctx->worker_pid = spawn_worker_process (
      "/usr/bin/gst-vlm-multistream-usecase-ipc", appctx->config_file,
      &appctx->ipc_fd, appctx->logger_level, appctx->profile_enabled);
...
scene_change_detection (VLMAppContext * appctx)
{
  ...
  for (auto & idx : appctx->sd_streams) {
    ...
    if (changed) {
      ...
      g_print ("Scene change detected for idx %d, push to "
...
main (gint argc, gchar * argv[])
{
  ...
  while (TRUE) {
    ...
    std::queue<guint> empty;
    std::swap (appctx.stream_scheduler, empty);
    for (guint x = 0; x < appctx.num_streams; x++) {
